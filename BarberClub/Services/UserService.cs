...
public class UserService(ProjectDbContext context, IWebHostEnvironment webHostEnvironment) : IUserService
{
    ...
    public async Task<User?> UpdateUserAsync(int userId, UserUpdateRequest request)
    {
        ...
        if (request.ProfilePictureFile != null && request.ProfilePictureFile.Length > 0)
        {
            ...
            string uniqueFileName = Guid.NewGuid().ToString() + Path.GetExtension(request.ProfilePictureFile.FileName);
            ...
            {
                await request.ProfilePictureFile.CopyToAsync(fileStream);
            }
